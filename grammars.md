...
- **禁止this指向全局对象**(在顶层调用函数this为undefined)
...
用var声明属于显示声明, 会在预编译阶段创建一个全局或局部变量

未用var声明直接赋值属于隐式声明, 实际上会查找当前作用域链, 找到对应属性则进行修改, 找不到时会在最上层(window)对象上创建属性并赋值;因此在执行阶段才存在

已声明的变量不可删除, 使用let, const声明的变量不可重复声明, 否则会报错; 隐式声明的变量可以删除, 即删除全局对象的属性

**同时显示声明和隐式声明了一个变量, 它即成为了一个全局属性, 且不可删除**
